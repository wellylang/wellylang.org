...
__all__ = [
    ...
    'sniff_encoding',
    'ValidationError',
    'PY3',  # PY3 is deprecated, and may be removed in future versions
]
...
def xmldecl(version='1.0', encoding=None, standalone=None):
    ...
    if encoding is not None:
        ...
        if isinstance(encoding, _binary_type):
            encoding = encoding.decode('ascii', 'replace')
        if isinstance(encoding, _text_type) and EncName_re.match(encoding):
    ...
    if standalone is not None:
        if standalone is True or standalone == 'yes':
            ...
            raise ValidationError('Bad standalone value in XML declaration',
                                  0, 1, 1, [])
    return u'<?xml version="%s"%s%s ?>' % (version, encodingdecl, sddecl)
...
def fix_xmldecl(xml, encoding=None, add_encoding=False, default_version='1.0'):
    ...
    if bomless_utf16_re.match(enc):
        ...
        prefix = encode(u'K')[:-len(encode(u'K'))]
    ...
    group = lambda s: b'(' + s + b')'
    oneof = lambda opts: b'(?:' + b'|'.join(opts) + b')'
    charset = lambda s: oneof([L(c) for c in s])
    all_until = lambda s: b'(?:(?!' + s + b').)*'
    upper = charset('ABCDEFGHIJKLMNOPQRSTUVWXYZ')
    lower = charset('abcdefghijklmnopqrstuvwxyz')
    digits = charset('0123456789')
    punc = charset('._-')
    Name = b'(?:' + (oneof([upper, lower]) +
                     oneof([upper, lower, digits, punc])) + b'*)'
    Ss = charset(' \t\r\n\f') + b'*'  # optional white space (inc. \f)
    Sp = charset(' \t\r\n\f') + b'+'  # required white space (inc. \f)
    ...
    if m:
        ...
        if m4:
            return decode(
                ...
                (attrs.get(ENCODING) if ENCODING in attrs else b'') +
                (attrs.get(STANDALONE) if STANDALONE in attrs else b'') +
...
def fix_doctype(html):
    ...
    NameStartChar = (
        u'[:A-Z_a-z\xC0-\xD6\xD8-\xF6\u00F8-\u02FF\u0370-\u037D' +
        u'\u037F-\u1FFF\u200C-\u200D\u2070-\u218F\u2C00-\u2FEF' +
        u'\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD]')
...
def xhtmlify(html, encoding=None,
    ...
    unicode_input = isinstance(html, _text_type)
    if unicode_input:
        html = html.encode(encoding, 'strict')
    if not isinstance(html, _binary_type):
        raise TypeError("Expected %s, got %s" %
                        (_binary_type.__name__, type(html)))
...
def test(html=None):
    ...
    try:
        ...
        print(xhtml)
        raise
    assert isinstance(xhtml, _binary_type)
    xhtml = xhtml.decode(sniff_encoding(xhtml))
    xmlparse(re.sub(u'(?s)<!(?!\[).*?>', '', xhtml))  # ET can't handle <!...>
    if len(sys.argv) == 2:
...
def xmlparse(snippet, encoding=None, wrap=None):
    ...
    unicode_input = isinstance(snippet, _text_type)
    if not encoding:
    ...
    if wrap is None:
        wrap = not snippet_text.startswith((u'<?xml', u'\ufeff<?xml'))
    if wrap:
        input_text = u'<document>\n%s\n</document>' % snippet_text
...
def sniff_encoding(xml):
    ...
    if isinstance(xml, _binary_type):
        xmlbytes = xml
    elif isinstance(xml, _text_type):
        xmlbytes = xml.encode('utf-8')
    else:
    ...
    punc = charset('._-')
    name = b'(?:' + (oneof([upper, lower]) +
                     oneof([upper, lower, digit, punc])) + b'*)'
    Ss = charset(' \t\r\n') + b'*'  # optional white space
    Sp = charset(' \t\r\n') + b'+'  # required white space
    joinbytes = b''.join
    ...
    EncodingDecl = joinbytes([
        ...
        Eq,
        oneof([
            L("'") + b'(?P<enc_dq>' + name + b')' + L("'"),
            L('"') + b'(?P<enc_sq>' + name + b')' + L('"')
        ])
    ])
...
def sniff_bom_encoding(xml):
    ...
    if not isinstance(xml, _binary_type):
        raise TypeError('Expected %s, got %r' % (_binary_type.__name__,
                                                 type(xml)))
    # Warning: The UTF-32 codecs aren't present before Python 2.6...
    # See also http://bugs.python.org/issue1399
    enc = {
        b'\x00\x00\xFE\xFF': 'utf_32',  # UCS4 1234, utf_32_be with BOM
        b'\xFF\xFE\x00\x00': 'utf_32',  # UCS4 4321, utf_32_le with BOM
        b'\x00\x00\xFF\xFE': 'undefined', # UCS4 2143 (rare, give up)
        b'\xFE\xFF\x00\x00': 'undefined', # UCS4 3412 (rare, give up)
        b'\x00\x00\x00\x3C': 'UTF_32_BE', # UCS4 1234 (no BOM)
        b'\x3C\x00\x00\x00': 'UTF_32_LE', # UCS4 4321 (no BOM)
        b'\x00\x00\x3C\x00': 'undefined', # UCS4 2143 (no BOM, give up)
        b'\x00\x3C\x00\x00': 'undefined', # UCS4 3412 (no BOM, give up)
        b'\x00\x3C\x00\x3F': 'UTF_16_BE', # missing BOM
        b'\x3C\x00\x3F\x00': 'UTF_16_LE', # missing BOM
        b'\x3C\x3F\x78\x6D': 'ASCII',
        b'\x4C\x6F\xA7\x94': 'CP037',     # EBCDIC (unknown code page)
    }.get(xml[:4])
    if enc and enc == enc.lower():
        return enc
    if not enc:
        if xml[:3] == b'\xEF\xBB\xBF':
            return 'utf_8_sig'  # UTF-8 with these three bytes prefixed
        elif xml[:2] == b'\xFF\xFE':
            return 'utf_16_le'
        elif xml[:2] == b'\xFE\xFF':
            return 'utf_16_be'
        else:
            enc = 'UTF-8'  # "Other"
    return enc
